...
export class ConsulModule {
  static forRoot({ name, ...options }: ConsulModuleOptions & { name?: string; }): DynamicModule {
  ...
  static forRootAsync(
    { name, ...options }: ConsulModuleAsyncOptions & { name?: string; },
  ): DynamicModule {
      ...
      return [
        {
          provide: CONSUL_MODULE_OPTIONS(name),
          useFactory: async (
            optionsFactory: ConsulModuleOptionsFactory,
